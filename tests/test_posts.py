...
def create_post_default_published_true(authorized_client,test_user,test_posts):
    res = authorized_client.post("/posts/", json={"title": "new title", "content": "new content"})
    created_post = schemas.Post(**res.json())
...
def test_update_post_nonexistent(authorized_client,test_posts,test_user,test_user2):
    data = {
        "title": "updated title",
        "content": "updated content",
        "id": 8888,
